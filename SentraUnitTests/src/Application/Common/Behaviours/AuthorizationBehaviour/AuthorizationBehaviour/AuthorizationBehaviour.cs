...
namespace CleanArchitecture.Application.Common.Behaviours
{
    public class AuthorizationBehaviourTests
    {
        ...
        [Fact]
        public async Task Handle_WithMultipleRolesAndUserInOneRole_AuthorizesSuccessfully()
        {
        ...
        public async Task Handle_WithEmptyRolesString_ContinuesWithoutError()
        {
            ...
            // Assert
            result.Should().NotBeNull("response should be returned even with empty roles");
        }
        ...
        public async Task Handle_WithEmptyPolicyString_ContinuesWithoutError()
        {
            ...
            // Assert
            result.Should().NotBeNull("response should be returned even with empty policy");
        }
        ...
        {
            // Business Context: Ensures null user handling doesn't bypass security
            // Arrange
            var request = new TestRequest();
            _mockUser.Setup(u => u.Id).Returns((string?)null);
            ...
            // Act & Assert
            await _behaviour.Invoking(b => b.Handle(request, () => Task.FromResult(new TestResponse()), CancellationToken.None))
                .Should().ThrowAsync<UnauthorizedAccessException>("null user should be treated as unauthenticated");
        }
        ...
        {
            // Business Context: Ensures proper error propagation from identity service
            // Arrange
            var request = new TestRequest();
            _mockUser.Setup(u => u.Id).Returns("test-user-123");
